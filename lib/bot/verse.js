...
function getVerseRange(chapterNo, verseStart, verseEnd, action = 'index', source = 'quran') {
    ...
    const tafsirs = idTafsirs[1].verses.filter((v) => v.chapter === chapterNo && v.verse >= verseStart && v.verse <= verseEnd);
    return {
        source,
        action,
        data: {
