...
async function testQuestion(text) {
    ...
    const answer = await (0, bot_1.getAnswer)(resp);
    ...
    (0, logger_1.debug)('[TEST] answer details:', (_b = answer.data) === null || _b === void 0 ? void 0 : _b.translations);
...
async function testCache() {
    ...
    (0, assert_1.default)(answer.data.chapter.id === 2 && answer.data.verses.length === 10);
    // cek cache
    cache = await (0, bot_1.getCache)();
    (0, assert_1.default)(cache['UNKNOWN'].length === answer.data.chapter.verses - 10);
    for (let i = 2; i <= 5; i++) {
        // lanjut
        answer = await testQuestion('lanjut');
        (0, assert_1.default)(answer.data.chapter.id === 2 && answer.data.verses.length === 10);
        // cek cache lagi
        cache = await (0, bot_1.getCache)();
        (0, assert_1.default)(cache['UNKNOWN'].length === answer.data.chapter.verses - 10 * i);
    }
}
